...
def load_mock_candidates() -> list[list[tuple[str, float]]]:
    """
    Returns mock candidate lists for testing the TransformerDecoder.
    Each inner list corresponds to one "time step" or "window" in a sentence.
    """
    raw_candidates = [
    ...
    return [[(word.lower(), score) for word, score in window] for window in raw_candidates]

def main():
    candidate_lists = load_mock_candidates()
